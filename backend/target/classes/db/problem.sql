...
CREATE TABLE IF NOT EXISTS `question_bank` (
    ...
    `jobType` varchar(32) NOT NULL COMMENT '岗位类型：FRONTEND(前端)、BACKEND(后端)、ALGORITHM(算法)等',
    `level` varchar(32) NOT NULL COMMENT '难度等级：BEGINNER(入门)、INTERMEDIATE(中级)、ADVANCED(高级)',
    `category` varchar(32) NOT NULL COMMENT '分类：BASIC(基础)、PROJECT(项目)、INTERVIEW(面试)',
    `type` varchar(32) NOT NULL COMMENT '题目类型：ALL(全部)、CHOICE(选择题)、JUDGE(判断题)、PROGRAM(编程题)',
...
CREATE TABLE IF NOT EXISTS `submission` (
    ...
    `isDelete` tinyint NOT NULL DEFAULT '0' COMMENT '是否删除',
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='提交记录';
...
CREATE TABLE IF NOT EXISTS `programming_problem` (
    `id` bigint NOT NULL COMMENT '关联problem表的id',
    `testCases` text NOT NULL COMMENT '测试用例，JSON格式',
    `sampleInput` text DEFAULT NULL COMMENT '示例输入',
    `sampleOutput` text DEFAULT NULL COMMENT '示例输出',
    `timeLimit` int NOT NULL DEFAULT 1000 COMMENT '时间限制(ms)',
    `memoryLimit` int NOT NULL DEFAULT 256 COMMENT '内存限制(MB)',
    `templateCode` text DEFAULT NULL COMMENT '模板代码',
    PRIMARY KEY (`id`),
    FOREIGN KEY (`id`) REFERENCES `problem` (`id`)
...
CREATE TABLE IF NOT EXISTS `programming_submission` (
    ...
    `language` varchar(32) NOT NULL COMMENT '编程语言',
    `code` text NOT NULL COMMENT '代码',
    `status` varchar(32) NOT NULL COMMENT '状态：ACCEPTED/WRONG_ANSWER/TIME_LIMIT_EXCEEDED等',
...
CREATE TABLE IF NOT EXISTS `choice_judge_submission` (
    ...
    `submitTime` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='选择判断题提交记录'; 